    ...
    <v-row justify="center" align="center">
        <v-col cols="12">
            ...
            <v-card class="mb-3">
                    ...
                    <v-row v-for="i in count" :key="i" justify="start">
                        ...
                        <v-col cols="2">
                            <v-select v-model="defaultLogoA[i - 1]" :items="logoData" item-title="name" item-value="id"
                                density="compact" label="Default LogoA"
                                :disabled="Array.isArray(logoImageA[i - 1]) && logoImageA[i - 1][0] !== undefined"></v-select>
                        </v-col>
                        <v-col cols="1">
                        ...
                        <v-col cols="2">
                            <v-select v-model="defaultLogoB[i - 1]" :items="logoData" item-title="name" item-value="id"
                                density="compact" label="Default LogoB"
                                :disabled="Array.isArray(logoImageB[i - 1]) && logoImageB[i - 1][0] !== undefined"></v-select>
                        </v-col>
                        <v-col cols="1">
                ...
                <v-card-actions>
                    <v-spacer />
                    <v-btn color="red darken-3" @click="generateImage"> スライド作成 </v-btn>
                    <!-- <v-btn color="light-blue" @click="generateImageNormalThumbnail('STORM')">サムネイル作成</v-btn> -->
                    <!-- <v-btn color="blue-darken-4" @click="generateImageNormalThumbnail('FRONTIER')">サムネイル作成</v-btn> -->
                    <v-btn variant="flat" color="blue-darken-4"
                        @click="generateImageThumbnail('STORM')">STORMサムネイル作成</v-btn>
                    <v-btn variant="flat" color="yellow"
                        @click="generateImageThumbnail('FRONTIER')">FRONTIERサムネイル作成</v-btn>
                </v-card-actions>
            </v-card>
...
const thumbnailImageBlueUrl = ref('/ink-wave/todays_result/thumb.jpg')
const thumbnailImageAquaUrl = ref('/ink-wave/todays_result/thumb_end_A.jpg')
const thumbnailImageF = ref('/ink-wave/todays_result/thumb_end_F.jpg')
const thumbnailImageS = ref('/ink-wave/todays_result/thumb_end_S.jpg')
...
// const generateImageNormalThumbnail = async (type: string): Promise<Boolean> => {
//     canvas.value = document.querySelector('#imageCanvas');
...
//     const baseImage = new Image();
//     if (type == 'STORM') {
//         baseImage.src = thumbnailImageBlueUrl.value;
//     } else {
//         baseImage.src = thumbnailImageAquaUrl.value;
//     }
...
//         } else {
//             context.value.fillStyle = '#00B1C5';
//         }
...
const generateImageThumbnail = async (type: string): Promise<Boolean> => {
    canvas.value = document.querySelector('#imageCanvas');
    if (!canvas || !canvas.value.getContext) return false;
    context.value = canvas.value.getContext('2d')
    ...
    const baseImage = new Image();
    if (type == 'STORM') {
        baseImage.src = thumbnailImageS.value;
    } else {
        baseImage.src = thumbnailImageF.value;
    }
    ...
    baseImage.onload = () => {
        ...
        // }
        context.value.font = `bold ${fontSize}px 'Noto Sans JP', sans-serif`;
        const dayX = (295 + 1044 - context.value.measureText(dayNo.value).width) / 2 + 1044 / 2;
        context.value.fillStyle = 'white';
        context.value.fillText(dayNo.value, dayX, 220);
        ...
        const x = (295 + 1044 - context.value.measureText(seasonNo.value).width) / 2 + 1044 / 2;
        context.value.fillText(seasonNo.value, x, 116);
        console.log('seasonTextWidth:' + seasonTextWidth);
        console.log('x:' + x);
